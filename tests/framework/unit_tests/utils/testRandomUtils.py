...
def checkTrue(comment,value,expected):
  ...
  if value == expected:
    ...
    print(f"({randomUtils.stochasticEnv})","checking answer",comment,value,"!=",expected)
    results["fail"] += 1
    return False
...
def checkArray(comment,check,expected,tol=1e-7):
  ...
  if not same:
    print(f"({randomUtils.stochasticEnv})","checking array",comment,"did not match!")
    results['fail']+=1
    return False
  else:
    results['pass']+=1
...
def checkType(comment,value,expected,updateResults=True):
  ...
  if type(value) != type(expected):
    print(f"({randomUtils.stochasticEnv})","checking type",comment,value,'|',type(value),"!=",expected,'|',type(expected))
    if updateResults:
      results["fail"] += 1
    return False
  else:
...
for env in ['crow', 'numpy']:
  print('Running tests for stochastic environment',env,'...')
  randomUtils.setStochasticEnv(env)
